...
strong audit judgment fails the screen. A saturation checkpoint after two
consequential-free batches of pending members lets the remaining pending
members stay inventoried and unread; any later consequential batch, screening
round or policy change removes that effect.
"""
...
def _audit(reading, version_id, work_id, collection_id):
    audit = (reading or {}).get("batch", {}).get("audit") if reading else None
    if audit is None:
        return obligation("screening_audit_reading_missing", "Read this sampled excluded paper with an audit judgment.",
                          version_id=version_id, work_id=work_id, collection_id=collection_id)
    if audit["relevance"] == "strong":
        return obligation("screening_audit_failed", "A sampled excluded paper is strongly relevant; re-screen the excluded members in a new round.",
                          version_id=version_id, work_id=work_id, collection_id=collection_id)
    return None


def member_obligations(records, collection, inventory, matched):
    """Obligations of one collection's members under screened-v1; inventory items carry reading_id."""
    ...
    obligations, read_doctrine, unread_pending = [], {}, 0
    ...
    for item in inventory:
        ...
        if disposition in ("promote", "doctrine"):
            ...
            if covered:
                ...
                obligations.append(missing)
        elif work_id in sampled:
            found = _audit(matched.get(version), version, work_id, collection["id"])
            if found is not None:
                obligations.append(found)
    population = {}
    for item in inventory:
...
def reference_obligations(records, profile, version, work, reading, qualified, sample, paths):
    """Obligations of one Tier 3 version at abstract depth, under either policy."""
    ...
    if disposition == "pending":
        return [missing] if reading is None and not qualified and not saturation_covers(records) else []
    if work["work_id"] in sample:
        found = _audit(reading, version, work["work_id"], None)
...
def record_screening_checkpoint(store, payload, *, expected_revision, request_id):
    ...
    def prepare(records, value):
        ...
        for batch in batches[-2:]:
            if batch["consequential"] or any(m["disposition"] != "pending" for m in batch["members"]):
                raise ResearchError("invalid_screening", "Saturation needs two batches of pending members with no consequential item",
                                    {"batch_id": batch["id"]})
...
def policy_report(store, *, policy=None, reference=None):
    ...
    for summary in cohort.get("collections", []):
        ...
        collections[summary["collection_id"]] = {
            "members": summary["member_count"], "read": sum(i["reading_id"] is not None for i in items),
