...
receipt through the managed path with its intent reconciliation. An interrupted
managed deposit finishes on the next run of the same command: the reconciliation
reads the record on Zenodo, keeps every step that landed, and sends the step
that never landed to that same record. A production deposit runs the offline
citation check first, prints the report when the check fails, and continues. A
production publish still requires `--confirm-publish`.
