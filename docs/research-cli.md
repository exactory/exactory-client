...
A media type that declares JSON requires strict JSON: finite numbers, no duplicate fields, no lone surrogates and depth of at most 40, measured on brackets outside strings. Bytes of another media type, and string values, that begin with `{`, `[` or `"` are JSON when a JSON decoder reads them (Python's `json` module, with integers of any length), and then the same strict rules apply, because as text their decoded strings would not reach the privacy checks. Bytes that no JSON decoder reads are delivered as exact text after the raw-byte privacy check, so a plain-text transcript that begins with a bracket does not fail with `invalid_json` or the depth bound. The privacy check does not decode escape sequences in such text.
