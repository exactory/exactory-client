...
Under `lineage-v1` the manuscript gate also checks the pinned bibliography for every `lineage` and `classic` entry. It reads the same citation evidence as `citation_accounting`. A BibTeX entry cites the work when it carries the work's arXiv identifier in family form, its DOI or a DOI of a provider alias, as a whole identifier. A bibliography that cites one version cites the work. An entry also cites the work when its title field equals the work's title of two or more words; a one-word title is too common to stand as evidence. Another bibliography format cites the work when its text carries an identifier or the title as whole words. A work with no such evidence keeps `lineage_citation_missing` until the bibliography carries it. Since 0.43.0 a DOI that only starts another DOI, and a title inside a longer title, are not evidence. The bibliography is read for citation evidence, not validated: undecodable bytes become replacement characters instead of a failure.
