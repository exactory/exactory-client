...
`batches --depth abstract --size N --destination DIR` writes the current unread abstracts (cohort members first, then Tier 3 references) as `batch-NNN.json` files with the saved text, the exact link, and a `README.json` naming the notes shape that `read-batch` accepts. It reads the store and writes only under the new destination directory; a reader agent works from one file and returns one notes file, and a single coordinator submits it with `read-batch`.

Under `screened-v1` the cohort gate requires a screening for every member (`screening_missing`), an abstract reading for every `promote`, `doctrine` and (until saturation) `pending` member, an audit reading with `audit: {relevance, reason}` for a deterministic sample of at most 150 excluded members per collection (`screening_audit_reading_missing`; a `strong` audit judgment is `screening_audit_failed` until a new round promotes the paper), and at least 8 read `doctrine` or `promote` members per calendar month of the window, or every member of a smaller month (`doctrine_coverage_missing`). Tier 3 references need a screening keyed by family with `context`; excluded references are sampled the same way. `require-fulltext` families and unresolved references keep their obligations whatever their screening says. `batches --screen` exports the unscreened members for `screen-batch`.
