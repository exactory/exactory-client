...
A media type that declares JSON requires strict JSON: finite numbers, no duplicate fields and depth of at most 40. Bytes of another media type that begin with `{` or `[` (or `"`, for a string value or a round capture) keep the depth bound, measured on brackets outside strings. They are JSON when a JSON decoder reads them, and then the same strict rules apply. Otherwise they are delivered as exact text after the raw-byte privacy check, so a plain-text transcript that begins with a bracket no longer fails with `invalid_json`. Text that a decoder reads but that is not strict JSON, such as JSON with duplicate fields, is refused, because as text its decoded strings would not reach the privacy checks.
