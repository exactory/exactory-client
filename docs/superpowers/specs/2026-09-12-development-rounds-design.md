...
- `next.number` is `closes + 1`. `next.objective` is either identical to the configured objective (`objective_lineage: null`) or a wider statement with `objective_lineage: {previous_id, containment}` where `containment` explains why the new objective contains the previous one. Narrowing is refused as today (`objective_locked`).
...
- Applies the objective: when `objective_lineage` is present, the configuration target becomes `next.objective`, a new `research_objective` record is created with `predecessor` set to the previous objective id, and the previous objective stays retained. This is the only path that changes a research objective.
...
- `research_objective` records gain an optional `predecessor` (objective id). The configuration target is the latest.
...
- `standards` is inherited; it goes stale only when the population changes, as today.
