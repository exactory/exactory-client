...
Provide an explicit `select_cohort_abstract(store, payload, *, expected_revision, request_id)` operation for an existing versionless arXiv member obligation. Its payload is `{collection_id, work_id: FAMILY_ID, unresolved_assertion_id, selected_assertion_id, reason}`. The unresolved assertion must belong to that collection and member. The selected assertion must have an explicit version in the same unambiguous arXiv family, match the category and initial-publication window, and supply a complete original abstract with verified source and artifact bytes. This selects current cohort evidence; it does not establish the old capture's version. Preserve the old records, responses, page history, counts and enumeration state, and expose the historical unresolved provenance. Bind the current reading obligation to the selected exact source without automatically transferring a reading. A known version with a missing abstract needs evidence for that same version. This operation cannot resolve malformed or truncated responses, category/date conflicts, enumeration uncertainty, rate limits, or resource pauses. Original request replays keep their original results.
