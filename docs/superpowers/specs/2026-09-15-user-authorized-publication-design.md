...
The command prints the deposit state before it keeps the local record, so the DOI reaches the user whatever the store does. Two cases leave a store that holds no record of the deposit: a store that exists and refuses the write, and a store file that exists and does not open. In both, `export_workspace` rewrites `.exactory/deposit.json` from the store, so the written file holds a deposit the store does not know. Both print the one-line note, write the file, state that the file is the only local copy and that a projection export can replace it, and exit nonzero.

A lost response on the direct publish is the one request this command cannot offer to send again: the direct path saves no intent, so a second run publishes a second permanent record. That request names its own advice for an unreachable API, which says that the deposition printed above may be published already and that the user opens that record. Every other request keeps the plain advice to run the command again, including the managed publish, which resumes its saved intent through remote reads.
