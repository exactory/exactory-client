...
`select_managed_path` performs the three steps and returns three values: the store it opened, what the check returned, and, for a store file that exists and does not open, the `ResearchError` that opening it raised. The store comes back on the direct path as well, so a command that writes its own record reuses the store this call already opened, and one refusal is noted once. The third value is for a command that keeps a local record of what it did: `exactory-draft deposit` reads it to warn that `.exactory/deposit.json` holds the only local copy of the record, because a later projection export reads the store, and that store never opened. `exactory submit` and `exactory verify` keep no local copy, so they ignore it.
...
Preconditions of the managed path: a workspace with a store exists; `validate_submission` passes (the `deposited` publication gate is ready, exactly one production receipt matches the current bundle, and the submitted DOI or URL names that record); no saved submit intent of this binding has an unknown outcome. `check_submission_preconditions` holds all of them and returns the intent binding and the deduplication key, so the decision point and `submit_managed` run one set of checks over one snapshot. The pending-intent check belongs with the others because a stranded intent is a state of the workspace, not of the user's request: it raises `remote_reconciliation_required`, the command prints the skipped line and posts the request, and the intent stays in flight for `exactory reconcile`.
...
Citation report: inside a draft workspace (a directory tree holding `.exactory/draft.json`, which a study gains from `exactory-draft init`) the command still runs `exactory-check gate` before the send. A failing gate writes its message to stderr with the prefix `Citation report: ` and the command continues. A workspace with no draft marker runs no check, because the check reads the draft workspace's references file.
...
Managed path: `research_harness.zenodo.deposit`, which continues the saved intent of this exact binding. A run that follows a lost response reconciles the claimed step through remote reads first: a step the record shows landed is resolved from that read, and a step the record shows never landed is discarded, so the same run sends that one step to that same record. The `create` step of a first deposit has no record to read yet, so it reads the account's deposition listing for the intent's own token: one match resolves the step, and a listing that was read to its end with no match establishes that the creation never landed, so the claim is discarded and the same run creates the record. No match in a listing the reads did not finish settles nothing. The publish reads the record's `submitted` status (https://developers.zenodo.org), so only an unpublished record earns the publish again and one deposition is published once. The `create` step of a `--new-version` deposit is the one step no read names, because the lost response carried the new record's ID: the claim is discarded and the same run sends the new-version action again, which https://developers.zenodo.org documents as having no effect while the draft of the first call is unpublished. A step neither read settles keeps the claim and refuses with `remote_reconciliation_required`; the refusal names the request id and the pending step, and a record that no later read settles keeps it.

Direct path: the 0.36.0 flow. Create the deposition (or open a new version of the deposition recorded in `.exactory/deposit.json` when `--new-version` is set), upload `paper.pdf` and the optional sources archive, set the metadata, mark `paper.pdf` as the default preview, and publish when `--publish` is set. The PDF may live anywhere on disk; the abstract file is read as bytes, decoded as UTF-8, and must hold nonblank text. It is read as bytes because the publication bundle pins the file's own bytes: a text read translates CRLF to LF, which changes the description, and with it the fingerprint of the deposit intent, so an interrupted deposit stops resuming and creates a second record.
...
The command prints the deposit state before it keeps the local record, so the DOI reaches the user whatever the store does. Two cases leave a store that holds no record of the deposit: a store that exists and refuses the write, and a store file that exists and does not open. In both, a later `export_workspace` can replace `.exactory/deposit.json` from the store, which holds no record of this deposit. Both print the one-line note, write the file, state that the file is the only local copy and that a projection export can replace it, and exit nonzero.

An uncertain outcome on the direct publish is the one this command cannot offer to send again: the direct path saves no intent, so a second run publishes a second permanent record. That request names its own advice, which names the deposition and its draft URL, says that this record may be published already, and tells the user to open it. The advice carries the record itself, because stderr carries the advice to a caller that reads none of the lines on stdout. Two failures print it, because both leave the request unsettled: an unreachable API, and a status of 500 or above, which states no outcome for the request. A status below 500 is the server's decision on the request it read, so it prints the status alone. Every other request keeps the plain advice to run the command again, including the managed publish, which resumes its saved intent through remote reads.
