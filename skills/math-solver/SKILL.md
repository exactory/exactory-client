...
Five rules hold throughout. The controller contract preserves the original claim, requested proof or decision standard, deliverables, and limits; a local statement or finish never replaces them. No result is declared before the novelty check has run for that exact statement (stage 3). No move is journalled under a strategy before that strategy's study record exists; `journal add` refuses one (stage 5). Every move is reserved with `search begin`, then journalled with `journal add` before the next move starts (stage 5). The cash-out (stage 7) starts only from one of the three native rules stage 7 names, and one of them fires within the budget the harness enforces.

Every command below is `exactory-math ...`, which is on PATH while this plugin is enabled, run from the directory the user is working in. The controller root defaults to `attack/`; admitted native workspaces live under `attack/<slug>/`. Give `--attack-root DIR` before the command when it lives elsewhere. Controller mutations use `search`, an exact JSON spec, the current expected revision, and a unique request ID. `search status` and `search next` are read-only. The strategy, entry, study, and reference files this document names are the skill's own files, not the user's: run `exactory-math skill-dir` once, and read each of them under the directory it prints.

## Stage 0: resume the original objective

A session can end before the objective does. The controller event record is the authority across all admitted nodes; each native record is the save for its local stage. Nothing needed for continuation is held only in the session.

Resume, instead of starting at stage 1, when the input asks to resume, restart, or continue, or when the session-start hook reports a managed objective. Run `search status --json`, then `search next --json`, and perform exactly the returned controller action. This includes an open objective whose native attacks all have `units/FINISHED.json`: local completion can leave another obligation, result review, root audit, or required deliverable pending. For the selected node, also run `status <slug>` and read `tasks.json`; these give local stage detail but do not override `search next`. A recorded pause survives status checks and session resume. Only an explicit authorized `search resume` clears it. Existing unmanaged work requires explicit reviewed adoption; never migrate or resume it automatically.
...
Produces: the immutable objective contract and, after reviewed admission, the first native workspace. Write the exact `Contract` from `SEARCH.md#contract-claims-and-scopes`, including the original claim, requested outcome (`proof` or `decision`), proof policy, deliverables, and resource policy. Run `search init --spec contract.json`, then follow the proposal, review, and admission flow in `SEARCH.md#proposal` and `SEARCH.md#review-and-provenance`. The normal proposal limits are explicitly 24 moves, 24 runs, 300 seconds, and one worker. Choose a kebab-case `attack_slug` from the claim's objects and quantities. `search admit` initializes `attack/<slug>/` with the native skeleton; do not call legacy `init` to create a managed node.

## Stage 2: set the problem

Produces: the controller `Claim` and the admitted node's `problem.json`. Validates: reviewed proposal admission first, then `check-problem <slug>`, which requires every key present, no empty strings, and `direction` and `mode` from their allowed sets. `problem.json.claim` must preserve the admitted claim. A changed statement is a separately reviewed route, coverage node, continuation, or standalone proposal; it never silently edits the original objective.
...
1. Search by statement, not by name. Use the claim's objects and quantities as terms in every source of the first tier of `references/sources.md` (the preprint server, the citation graph, the reviews databases, and the problem's own database when it has one), one query per source at the floor, then in the second tier (question-and-answer sites, research blogs, general web search) when the first tier misses. When the problem has a formal statement in a formalisation repository (`references/sources.md`, section 3), that statement is the canonical one: record it in `study/problem.md` and read `claim` against it quantifier by quantifier.
...
5. A statement already solved in the literature ends the local research route on that statement. Record the exact source in `novelty.md`; `finish <slug>` may record the native stage 3 literature finish. Then return the result and every remaining deliverable to the controller objective. A literature hit does not create accepted proof evidence, change a requested formalization into optional work, or close the root. If the original request includes formal verification, propose and admit the exact missing formal bridge or verification node and continue it. A neighbouring open statement is a separately reviewed logical proposal that preserves the original objective; it is not an automatic native child or a substitute target.
...
A plan step that needs a second record first writes the exact claim, contribution, logical predecessor, inherited evidence, account mode, success and failure criteria, and native-versus-logical lineage as a proposal under `SEARCH.md#proposal`. Independent review decides whether it is a prerequisite, coverage, continuation, alternative, standalone result, or native child. When `native_parent` is approved, `search admit` creates the native child and its `parent.json`; do not rerun legacy `init --from`, which refuses inside a managed objective with migration guidance. Native depth remains one level and the native parent must be unfinished and not itself a child. Deeper relationships use `logical_predecessor` and checkpoint lineage.

A child proof contributes to its parent only after the exact child claim and evidence are checkpointed and independently accepted, and the parent's dependency and implication or route bridge are themselves explicitly checked and accepted under `SEARCH.md#checkpoints-and-accepted-results`. A local child finish or `parent.json` link supplies neither implication nor proof acceptance. A stalled child leaves the precise assumption or route obligation open.

### The move loop

One move is one application of one entry.

Before the move: read the entry's Trigger against the `shape` fields of `problem.json`. The move exists only when the fields the trigger names match, and the journal line lists those fields under `trigger_features`. Reserve it first with `search begin NODE --spec begin.json`; the spec is the exact strategy, entry, pass, trigger fields, and step citations defined in `SEARCH.md#metered-execution-and-native-integration`.

During the move: carry out the entry's Action, reading the entry's Failure signal after each step. Before launching any process for a finite question, require the actual version 2 finite computation proposal and its independent review. The review must bind a legitimate basis (`root_finite_scope`, proved `finite_residue`, necessary `diagnostic`, independently significant `standalone`, or `counterexample`), exact finite domain, inspected saved evidence, information value, outcome/action map, stopping condition, and bounded producer/checker plan from `SEARCH.md#computation-admission-and-mandatory-interpretation`. Cheap, reproducible, already implemented, or independently checked work is not thereby necessary. A necessary bounded diagnostic remains admissible. A prefix/tail relabelling with an unresolved infinite or named tail is not a finite reduction. Tests may consume a saved certificate, but a test that secretly calls its producer is another run and needs its own admitted role.
...
## Stage 6: verify under frozen inputs

Produces: an editable `deterministic/<step>-<n>/` specification, immutable reviewed inputs, a controlled run with captured outputs, and an interpreted result. The version 2 computation contract and run schemas are in `SEARCH.md#computation-admission-and-mandatory-interpretation` and `SEARCH.md#metered-execution-and-native-integration`; do not duplicate or weaken them locally. Generic tasks use `search run NODE --spec run.json`. Native `verify lean <slug> <step-dir>` and `verify certificate <slug> <step-dir>` remain the legacy-facing commands, but inside a managed objective they route through the same reservation, frozen snapshot, deadline, one-worker account, exact output, and reconciliation path. They read an independent `verification-review.json` that binds the exact input subject. `step.json` declares the environment and dependencies and, for Lean, the exact requested declaration and `requested_type`.

The executor gives each workload at most its admitted timeout, with the templates fixed at 300 seconds and one worker; it meters at most 24 command units on the normal account. It captures immutable source bytes and modes, declared external dependencies, exact argv, stdout, stderr, outputs, and native inspection evidence. Shared external dependencies remain an explicitly trusted read-only boundary, not an OS-hermetic sandbox. The current release does not measure hidden internal solver calls, CPU, RAM, or a total byte budget, and it does not provide immediate cancellation of an already owned job. A pause blocks new launches; the controlled deadline still bounds the owned workload.
...
A successful process is only execution evidence. `search interpret` must match its actual terminal record to the admitted outcomes; `UNKNOWN`, timeout, failed, cancelled, indeterminate, or inconclusive results cannot become theorem candidates. Proof credit requires a frozen evidence manifest, checkpoint, independent result review, exact policy and declaration audit, and `search accept` under `SEARCH.md#immutable-evidence-manifests`. A certificate needs an independent checker and a completeness argument; Lean needs the exact requested declaration, requested type, toolchain inventory, successful build and inspection, and permitted axioms. Until acceptance, the journal and local unit label the result as evidence.
...
1. Run `stall <slug>`. It writes `units/INVENTORY.md`: the walk, then every journal move, grouped by strategy, marking the ones whose failure signal fired, because a fired signal leaves what the strategy's Failure signal names, marking the move that locally closed the attack, and marking what each move paid, with the whole ledger summed at the top. This is local cash-out and creates no proof or execution authority.
...
3. Convert each candidate under `CASHOUT.md`: the claim test, the form (a full-proof or second-proof candidate for a locally closed attack, one of the partial forms otherwise), and the decomposition into units. Each surviving claim becomes `units/<n>/` with `unit.json` holding `statement` (one sentence), `form` (one of the labels `check-unit` accepts, listed in `CASHOUT.md`), `evidence` (a path relative to the workspace that exists: the proof text, or the deterministic step directory), `novelty` (the stage 3 record for this statement, citing `study/problem.md` and the unit-level queries), `moves` (the journal move numbers that produced it), and `costs` (the ledger the unit's evidence carries, summed from the `costs_paid` of those moves), which `check-unit` requires; the unit's files are listed in `CASHOUT.md`, and the ledger decides the form there. A computational unit is theorem-level only when its admitted node covers an objective obligation or has the controller's reviewed standalone significance and its evidence has been accepted. Necessary diagnostic evidence can be cited without becoming a theorem.
...
3. Run `finish <slug>`. It writes `units/FINISHED.json` with the numbers of the units that stand, and the native attack is locally done. An attack that ended at stage 3 runs `finish` as well: with no move and no inventory it accepts the workspace on a non-empty `study/problem.md` and `novelty.md`, and records the local outcome as solved-in-literature. Return the local result, accepted evidence, and all remaining obligation IDs to the controller. If `search next` reports another ready node, continue it. If it reports `finalize_root`, run the independent closure review and `search complete` using `SEARCH.md#final-closure-and-local-delivery`; only successful completion changes the root proof status. A required formalization or other original deliverable remains on this path even after a literature finish. Do not start an unrelated alternative while root-ready evidence awaits audit and completion.
