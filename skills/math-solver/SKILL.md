...
Execute the current composition strategy by strategy, in the order its id gives. `journal add` refuses a move under a strategy of the composition while an earlier strategy of it has no move, and a move that returns to a strategy the composition has left. For each strategy:
...
- `strategy` and `entry`: the strategy and the entry dispatched. `journal add` refuses a strategy outside the composition, and an entry the strategy's front matter does not list.
...
Three rules start the cash-out, and nothing else does: `budget` prints `stall due: yes`, which a closing move also makes (stage 5, Closing); `plan` prints no composition. `stall` checks them, refuses to write the inventory while none holds, and names the rule it found.
...
1. Run `stall <slug>`. It writes `units/INVENTORY.md`: every journal move, grouped by strategy, marking the ones whose failure signal fired, because a fired signal leaves what the strategy's Failure signal names, marking the move that closed the attack, and marking what each move paid, with the whole ledger summed at the top.
...
- A plan with one method and no composition: a strategy executed that is in no composition of `compositions.json`, a composition executed out of the order `ranking.json` gives, or a choice justified by cost or by what recent results used. `journal add` refuses the first two, so a journal that shows them was not written by `journal add`; the journal is written by that command only. Return to stage 4 and take the order the solver wrote.
...
- A cash-out offered with no rule that triggered it: units written while `budget` prints `stall due: no`, `plan` printed a composition, and the attack has not closed. `stall` refuses to write the inventory and `check-unit` refuses to run without it, so units on disk under those conditions were written around the harness. Return to stage 5.
