---
name: math-solver
description: Solve, disprove, or advance a stated mathematical proposition - set the problem, check novelty against the literature, plan strategy compositions, run the moves under a fixed budget, and cash out what stands as a result. Use when a mathematical proposition, conjecture, or open problem is stated and is to be proved, disproved, or advanced; when a problem is given by number from a problem list or database; or when a claimed bound, exponent, constant, or finiteness statement is presented as open.
...
The skill attacks a stated mathematical proposition end to end and runs without stopping to ask. It has two halves. The deterministic half is `harness/attack.py` (contract in `harness/SPEC.md`, usage in `harness/README.md`): it owns the workspace files, validates what is written into them, enumerates strategy compositions, enforces the move budget, and runs the certificate checks. The judgment half is this text: every judgment about the problem is written into a file that `attack.py` validates and reads.
...
5. A statement already solved in the literature ends the attack on that statement. Record where it is in `novelty.md`, then return to stage 2 with the nearest neighbouring statement the search left open, in a new workspace; when the search left no neighbour open, the attack ends here, and what is reported is `novelty.md` and `study/problem.md`.
...
Produces: `preconditions.json`, `compositions.json` written by the harness, and `ranking.json`. Validates: `plan <slug>`, then `rank <slug>`.
...
Run `plan <slug>`. It refuses to run while `study/problem.md` is missing or empty (stage 3). It rejects the file when a strategy is missing, a cited field does not exist in `problem.json`, a question the strategy file asks is unanswered, an answer names a question the file does not ask, the record carries a key other than `verdict`, `answers`, `note`, and `failed_after_move`, or a verdict contradicts its required answers. The verdict follows the questions the file marks `required`; an optional answer is recorded and does not change the verdict. Otherwise it writes `compositions.json` from every strategy whose verdict is not `no`, capped and filled by round over the leading strategy so that every admitted strategy leads a composition, under the rules in `strategies/README.md`: one to four distinct strategies; every verdict yes, with at most one unknown, which marks the composition as resting on an assumption; A before B whenever A lists B under `precedes`; no pair that one lists under `excludes`; ordered by yes count, then by the number of distinct components spanned, then by name order; at most 20. What it prints is the shortlist, not the order of execution.

Then write `ranking.json`: the same compositions, every one of the shortlist and nothing else, in the order the attack will take them. Each row names the composition by its id, the strategies joined by `+`, and carries `cites`, the fields and costs that put it at that place (a dotted path into `problem.json`, or `cost:<name>` from the vocabulary in `strategies/README.md`), and `reason`, one sentence. Run `rank <slug>`; it refuses an order that is not exactly the current shortlist, and a row that cites nothing. The order is the solver's and not a formula, because what a cost is worth depends on the problem: giving up effectivity is fatal when `shape.target_quantity` is an explicit constant and harmless when it is finiteness, so no fixed weight per cost is right for both. Take the compositions in the order `ranking.json` gives, starting at the first. Every move at stage 5 names a composition of this file. `fail` re-plans (stage 5), which makes a new shortlist, so `ranking.json` is written again and `rank` run again before the next move.
...
Execute the current composition strategy by strategy, in the order its id gives. For each strategy:
...
3. Read the strategy's Failure signal after every move under it. When it fires, the strategy ends: run `fail <slug> <strategy>`, which sets its verdict to no with a note and re-runs `plan`. Write `ranking.json` again over the new shortlist and run `rank` (stage 4). Then continue with the first composition of the new order that contains a strategy not yet executed. The outputs of the strategies already executed stand in the journal and feed the next composition.
4. When the strategy's plan reaches its last step with its output, hand the output to the next strategy in the composition, as the strategy's Composes with section states. When the last strategy of the composition ends with the claim proved or refuted, the attack has closed: go to Closing below. When it ends short of that, take the next composition of the order `ranking.json` gives.
...
- `composition`: the id of the composition being executed, its strategies joined by `+`, as `ranking.json` orders it. The id survives a re-plan; a rank does not.
- `strategy` and `entry`: the strategy and the entry dispatched.
- `trigger_features`: a list of field names, such as `["shape.quantifiers", "shape.known_bounds"]`.
- `action`: what was done, naming each deterministic step directory the move ran.
...
- `costs_paid`: what this move gave up, from the cost vocabulary in `strategies/README.md`, and empty when it gave up nothing. A declared cost never removes a strategy from the shortlist; `journal add` refuses the move that pays one the quadruple forbids, under the two gates that file names.
- `failure_signal_fired`: true when the entry's failure signal fired during the move.
- `problem_changed`: true when the move changed a quadruple field or a shape field.

When `problem_changed` is true, write the change to `problem.json` (a statement move changes `quadruple.statement`, a stage move `quadruple.stage`, a direction move `quadruple.direction`, a mode move `quadruple.mode`, and any move the shape fields it settled); when the statement moved, write into `output` the deduction from the new statement back to `claim`; re-run `check-problem`. A quantity attacked in place of the claim with no such move is a red flag below.

### The budget

The constants as the code enforces them: 8 moves per pass, 3 passes, 24 moves in all. `journal add` rejects a move whose number is not the next, a move in a spent pass (the next move starts the next pass, up to pass 3), and every move while a stall is due. A stall is due when the last three moves since the last `fail` all fired their failure signal, when pass 3 has used its 8 moves, or when 24 moves are used. Ending a strategy with `fail` starts the count again, so a failure signal ends the strategy and not the attack. Run `budget <slug>` at the start of every pass and after every move whose failure signal fired; when it prints `stall due: yes`, go to stage 7.

At the start of every pass after the first: when a move of the previous pass had `problem_changed` true, re-answer the precondition questions that cite the changed fields, except for a strategy whose record `fail` wrote: its `no` and its `note` stay, and its questions are not re-answered. Rewrite `preconditions.json`, run `plan`, write `ranking.json` again and run `rank`, and continue with the first composition of the new order that contains a strategy not yet executed. The `composition` of the moves that follow is one the current `ranking.json` orders.

### Closing

The attack has closed when a move's output is a proof of `claim` or a counterexample to it. Before stage 7:
...
2. `quadruple.direction` and `quadruple.mode` in `problem.json` state what was produced, journalled as a move with `problem_changed` true when they changed.
...
Then go to stage 7 with the closing rule. The formal check of a closed proof is the output of verify-formally-with-lean4 when the composition that closed the attack contains it as its last strategy; the full-proof unit records the formalisation when it ran (`CASHOUT.md`).
...
Three rules start the cash-out, and nothing else does: `budget` prints `stall due: yes`; `plan` prints no composition; the attack has closed (stage 5, Closing).
...
1. Run `stall <slug>`. It writes `units/INVENTORY.md`: every journal move, grouped by strategy, marking the ones whose failure signal fired, because a fired signal leaves what the strategy's Failure signal names, and marking what each move paid, with the whole ledger summed at the top.
...
4. Run the stage 3 novelty check with the unit's statement, then `check-unit <slug> <n>`. A unit that fails `check-unit` is corrected and checked again; a unit whose statement the novelty check finds in the literature, or that fails the claim test, returns to the inventory.

## Stage 8: write

Produces: one draft per unit directory that passed `check-unit`, and the evaluation of that draft; both are steps of this skill's flow. Validates: `check-unit <slug> <n>`, run again when drafting changes `unit.json`; no command reads the draft.

1. Draft. The unit directory is the evidence package: `unit.json`, the file at its `evidence` path, the novelty record, and the journal excerpt (the moves listed under `moves`, copied from `journal.jsonl`). The draft states the unit's claim as `statement` in `unit.json` reads, quantifier by quantifier. Every claim in the draft points to the proof text or to a deterministic step directory with `result.json`; a claim with neither behind it is removed before drafting. Every reference in the draft carries the identifier (DOI, arXiv id, or URL) recorded for it in `known` of `problem.json`, in `novelty.md`, or in a `study/` record. The venue follows the unit's position relative to the target, on the path or off to the side, as `CASHOUT.md` states.
2. Evaluate. Before the draft leaves the workspace, a reader who did not write it reads it against the evidence package under the self-check in `CASHOUT.md`, resolves every reference to its identifier, and compares the draft's statement with `unit.json`. The draft passes when every claim points to its evidence, every reference resolves, and the statement is the unit's; otherwise it returns to step 1 with the reader's findings, and `check-unit` is re-run when `unit.json` changed.

## Red flags

- A move in the journal with no trigger match recorded: it counts as a move whose failure signal fired; re-check `problem.json` before the next move.
...
- A computation cited with no code under `deterministic/`, or with no `result.json`: it is not evidence.
- Move 25 about to start: stall now; `journal add` refuses it.
- A plan with one method and no composition: a strategy executed that is in no composition of `compositions.json`, a composition executed out of the order `ranking.json` gives, or a choice justified by cost or by what recent results used. Return to stage 4 and take the order the solver wrote.
...
- A cash-out offered with no rule that triggered it: units written while `budget` prints `stall due: no`, `plan` printed a composition, and the attack has not closed. Return to stage 5.
