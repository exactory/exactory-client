...
- `ground`: the observation in this evaluation that the step answers, and the
  place that observation holds in the field: name the cohort papers or the open
  discussion that show it. A suggestion with no ground is a wish; a ground with
  no field context is a private note.
